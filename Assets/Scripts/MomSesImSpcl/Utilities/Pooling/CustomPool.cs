...
namespace MomSesImSpcl.Utilities.Pooling
{
    ...
    public sealed class CustomPool<T> : CustomPoolBase<Queue<T>, T> where T : PoolWrapperBase<T>, new()
    {
        #region Constructors
        /// <summary>
        /// <see cref="CustomPool{T}"/>.
        /// </summary>
        /// <param name="_InitialCapacity">The amount of objects to create, when this <see cref="CustomPool{T}"/> is created.</param>
        /// <param name="_MaxCapacity"><see cref="CustomPoolBase{C,T}.MaxCapacity"/>.</param>
        public CustomPool(int _InitialCapacity, int _MaxCapacity = int.MaxValue) : base(_MaxCapacity)
        {
